{ config, lib, ... }:
{
  config.programs.claude-code = lib.mkIf config.programs.claude-code.enable {
    context = ''
      Use nix devshells from `flake.nix` when possible.
      Use jujutsu/jj over git where possible
    '';
    settings = {
      mcpServers = {
        atlassian = {
